"""
Unit tests for Pure Minimal genre scraping.

Tests the scraper's ability to extract Pure Minimal mixes from MixesDB.
Style Code: PM
Expected Results: ~617 mixes available
"""
...
class TestPureMinimalScraping(BaseMixesDBTest):
    """Test class for Pure Minimal genre scraping."""
    
    GENRE_NAME = "Pure Minimal"
    STYLE_CODE = "PM"
    EXPECTED_MIN_RESULTS = 600  # Conservative estimate
    
    @classmethod
    def setUpClass(cls):
        """Set up Pure Minimal test environment."""
        super().setUpClass()
        print(f"🎵 Testing Pure Minimal genre (Style Code: {cls.STYLE_CODE})")
        print(f"🎯 Target: {cls.TARGET_TRACKLISTS} tracklists")
        print(f"📊 Expected minimum available: {cls.EXPECTED_MIN_RESULTS}")
        print(f"⚠️  Note: This genre has limited available mixes (~{cls.EXPECTED_MIN_RESULTS})")
    
    def test_01_genre_exists(self):
        """Test that Pure Minimal genre exists and is configured correctly."""
    ...
    def test_02_small_batch_scraping(self):
        """Test scraping a small batch of Pure Minimal mixes."""
        genre = self.get_genre_by_name(self.GENRE_NAME)
    ...
    def test_03_medium_batch_scraping(self):
        """Test scraping a medium batch of Pure Minimal mixes."""
        genre = self.get_genre_by_name(self.GENRE_NAME)
        self.assertIsNotNone(genre)
    ...
    def test_04_large_scale_scraping(self):
        """Test scraping large number of Pure Minimal mixes (target limited by availability)."""
        genre = self.get_genre_by_name(self.GENRE_NAME)
        self.assertIsNotNone(genre)
        
        all_mixes = []
        # For Pure Minimal, we adjust target to the available amount
        target_remaining = min(self.TARGET_TRACKLISTS, self.EXPECTED_MIN_RESULTS)
        batch_count = 0
        
        print(f"🚀 Starting large-scale scraping for {self.GENRE_NAME}")
        print(f"🎯 Target: {target_remaining} tracklists (limited by availability)")
        ...
        while target_remaining > 0 and batch_count < 50:  # Safety limit
            ...
            try:
                ...
                self.log_test_progress(len(all_mixes), min(self.TARGET_TRACKLISTS, self.EXPECTED_MIN_RESULTS), self.GENRE_NAME)
        ...
        print(f"🎯 Target completion: {(len(all_mixes)/min(self.TARGET_TRACKLISTS, self.EXPECTED_MIN_RESULTS)*100):.1f}%")
        print(f"⭐ Average quality: {avg_quality:.2f}")
        print(f"💾 Results saved to: {result_file}")
        
        # Note about limited availability
        if len(all_mixes) < self.TARGET_TRACKLISTS:
            print(f"ℹ️  Note: Pure Minimal has limited availability. Scraped {len(all_mixes)} out of ~{self.EXPECTED_MIN_RESULTS} available mixes.")
    
    def test_05_track_quality_validation(self):
        """Test the quality of individual tracks in Pure Minimal mixes."""
        genre = self.get_genre_by_name(self.GENRE_NAME)
        self.assertIsNotNone(genre)
